...
def run_mul(problem, path):
    print problem.start
    m, n = len(problem.walls.data), len(problem.walls.data[0])
    x1, y1 = problem.start[0]
    x1, y1 = n - y1 - 1, x1
    data = '%i %i\n%s\n%i %i\n%s\n' % (n, m, problem.walls, x1, y1, problem.start[1])
    open('../build/tmp.txt', 'w').write(data)
...
def greedy(problem, heuristic):
    return run_mul(problem, '../build/greedy.out')

def enumez(problem, heuristic):
    return run_mul(problem, '../build/enumerate.out')

def opt(problem, heuristic):
